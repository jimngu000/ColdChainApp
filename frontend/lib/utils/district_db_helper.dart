...
class DistrictDatabaseHelper {
    ...
    try {
      ...
      var districtDatabase =
          await openDatabase(path, version: 1,
            ...
            await db.execute("CREATE TABLE districts($colId INTEGER PRIMARY KEY, $colUserId INTEGER, $colName TEXT);"),
          );
  ...
  Future<int> insertDistrict(District district) async {
    ...
    return await db.insert('districts', district.toMap(),
      conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<int> updateDistrict(District district) async {
    Database db = await database;
    return await db.update("districts", district.toMap(),
      where: '$colId = ?',
